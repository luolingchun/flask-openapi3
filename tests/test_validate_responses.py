...
def test_app_config_level_validate_response(request):
    """
    Validation turned on at app level
    """
    test_app = OpenAPI(request.node.name)
    test_app.config["TESTING"] = True
    test_app.config["FLASK_OPENAPI_VALIDATE_RESPONSE"] = True
...
def test_abp_config_level_validate_response(request):
    """
    Validation turned on at app level
    """
    test_app = OpenAPI(request.node.name)
...
# NOTE: can't register APIView on APIBlueprint - missing feature?
# def test_apiview_abp_level_validate_response(request):
#     """
#     Validation turned on at app level
#     """
#
...
#             return body.model_dump(), 201
#
#     test_abp.register_api_view(test_api_view)
#     test_app.register_api(test_abp)
#
#
#     with test_app.test_client() as client:
